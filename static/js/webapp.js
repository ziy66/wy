...
function WebApp(){
	var that = this;
	var _hostname = 'https://www.yahugames.com';
	var _urlParamValue = {};
    ...
    */
	...
	//选项资料
	var _data = [
		{id:'wxFriend',name:'微信好友',image:'mobile/images/wechat/wx_logo.png'},
		{id:'wxMonments',name:'微信朋友圈',image:'mobile/images/wechat/wx_moments.png'},
		{id:'wxCollect',name:'微信搜藏',image:'mobile/images/wechat/wx_collect.png'}
	];
   ...
   */
	...
	function _openBrowser(url){
		if(that.isWebApp()){
			if(that.getVersion()){
//				url = url.replace(window.location.origin,_hostname);
				_callApp('open_url',url,arguments[1],arguments[2]);
			}else{
				window.open(url);
			}
		}else{
...
//				url = url.replace(window.location.origin,_hostname);
    ...
    function _openAppKeFu(username){
		...
		_callApp('open_appKeFu',username,'yahugroup','亚博客服组','1');
    }
...
//		alert(JSON.stringify(data));
	...
	function _getAppUrl(url,data){
		var get = [];
		for(var i in data){
			if(_data[i]==undefined||_data[i]==null||_data[i]==''||_data[i]==false){
				get.push(i+'='+data[i]);
			}
		}
		if(url.indexOf('?')==-1){
