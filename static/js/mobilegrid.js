 ...
 *	}
 */
function MobileGridColumn(config){
	var that = this;
	var _defConfig = {
		value:false,
		name:false,
...
function MobileGrid(config){
	...
	var _gridTemplat = [
		'<div class="m-grid">',
	    	'<div class="m-grid-header">',
	            ...
	            '<div class="m-grid-refresh flaticon-refresh57"></div>',
	        '</div>',
	...
	var _arrowTemplate = '<td align="center"><div class="m-grid-row-open iconfont icon-fangdajing"></div></td>';
	...
	function _init(){
		...
		if(!_$grid){
			...
			if(that.config.showIndex){
				columnArray.push(String.format(_thTemplate,'序号'));
			}
