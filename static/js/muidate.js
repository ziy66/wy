...
(function(MUI){
	function Datepicker(mui){
    	...
    	var dateTemplate = [
           	...
           	'				<div class="d-content-yesrmonth-last i-arrow-r iconfont icon-common_Arrow"></div>',
           	'				<div class="d-content-yesrmonth-next i-arrow-r iconfont icon-common_Arrow"></div>',
           	'			</div>',
           	'			<div class="d-content-week"></div>',
           	'			<div class="d-content-day"></div>',
           	'			<div>',
            '    		  	<div class="mui-btn mui-col-xs32-6 mui-btn--raised mui-btn--brown submit">确认</div>',
            '    		  	<div class="mui-btn mui-col-xs32-6 mui-btn--raised mui-btn--brown cancel">取消</div>',
           	'			</div>',
           	'		</div>',
           	'		<div class="d-content-year-wrap">',
           	'			<div class="d-content-year"></div>',
           	'			<div class="d-buttons-wrap">',
            '    		  	<div class="mui-btn mui-btn--raised mui-btn--danger back">返回</div>',
           	'			</div>',
           	'		</div>',
    	...
    	function init(){
    		if(!$datepicker){
		    	...
		    	//左右变更月份
		    	$datepicker.find('.d-content-yesrmonth-last').click(function(){
		    		changeMonth(-1);
		    	});
    	...
    	function activeYear(){
    		...
    		for(var i = 0,l = divs.length;i<l;i++){
    			if(divs.eq(i).html()==that.activeDate.getFullYear() ){
    				...
    				that.$yearWrap.scrollTop(divs.eq(i).offset().top-(divs.eq(i).height()*6));
    				break;
    			}
    		}
    		divs = null;
    	...
    	that.open = function($element){
    		...
    		MUI.overlay('on',$datepicker[0]);
    		$datepicker.parent().css('zIndex',99);
    		if(_type[that.format]==0){
        		activeDay();
    		}else if(_type[that.format]==1){
        		activeMonth();
