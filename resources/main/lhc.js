	...
	var url = apirooturl + 'lotterytimes';
	  $.post(url, {'lotteryname':lotteryname,'cptype':'lhc'}, function(data) {
		...
		if(data.sign==true){
			...
			if(yIndex > 1){
				loginCengBoxFn(lottery.lastFullExpect+'期已截止<br />当前期号<span style="color:red;">'+lottery.currFullExpect+'</span><br />投注时请注意期号',-1);
			}
			yIndex++;
	...
	$node.html('');
	  $.post(url, {'lotteryname':lotteryname}, function(data) {
        ...
        var yopentime = yopentimes_arr[1];
				...
				html += '<tr><td width="130">'+openinfo.expect+'</td><td width="46" class="c_red fb" style="padding:5px;">';
				for(var i=0;i<array.length;i++){
					var code=array[i];
					var className=['blue','red','blue','green'][f_color(code|0)];
					if(i==array.length-1){
						html +='<var>+</var><span class="'+className+'">'+code+'</span>';
					}else{
						html +='<span class="'+className+'">'+code+'</span>';
					}
...
var
		...
		kk = ((new Date).getFullYear() - 2018) % 12,
...
{
	var ll = document.getElementById('ssc_winning_sum').getElementsByClassName('number-box'), em, sp;
	if(ll[e])
	{
		em = ll[e].firstElementChild;
		sp = em.nextElementSibling;
		em.className = ['blue','red','blue','green'][f_color(num|0)];
