...
var getuserlevel = function(){
	var url = mobileapirooturl + 'userlevel';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
function checkAddUsername(obj){
	var uname = $(obj).val();
	var url = mobileapirooturl + 'checkaddusername';
	$.ajax({
		url: url,
		type: 'POST',
		dataType: 'json',
...
function addUser() {
	...
	var rebateid = way.get("addUser.rebateid");
	var url = mobileapirooturl + 'adduser';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
//用户签约
function contractLog( objectType ,vlues)
{
	if( objectType == "") {
		alt("契约类型不存在!",-1);
	}
	var url = mobileapirooturl + 'addcontractlog';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
function drawshare(objectType){
	if( objectType == "") {
		alt("契约类型不存在!",-1);
	}

	var url = mobileapirooturl + 'drawshare';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
function addSignuplink() {
	...
	var url = mobileapirooturl + 'addsignup';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
function signuplinkList() {
	...
	var url = mobileapirooturl + 'signuplinklist';
...
function delSignuplinkConfirm(id){
	...
	var url = mobileapirooturl + 'delsignuplink';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
function allUserList(isonline) {
	...
	var url = mobileapirooturl + 'memberList';
	var pagination = $.pagination({
		...
		success : function(data){
			if(data['root'] != null){
				...
				if($('.'+id).nextAll().hasClass('parent'+id)){
					var url = mobileapirooturl + 'getids';
					$.ajax({
						url : url,
						type : 'post',
						data : {
...
function allDownUserBetsList() {
	...
	var url = mobileapirooturl + 'downuserbetslist';
	...
	var pagination = $.pagination({
		...
		success: function(data,sucdata) {
			...
			$.each(data, function(index, val) {
				var html = '<tr id="'+val.trano+'"><td> <a href="javascript:getBillInfo(\''+val.trano+'\')">' + val.trano + '</a></td>' +
					'<td>' + val.username + '</td>' +
					'<td>' + val.cptitle + '</td>' +
					'<td>' + val.expect + '</td>' +
					'<td>' + val.playtitle + '</td>' +
...
function accountChange() {
	...
	var url = mobileapirooturl + 'downuserchangelist';
	var pagination = $.pagination({
...
function groupDeposit() {
	...
	var url = mobileapirooturl + 'downuserrechargeandwithdrawlist';
	var pagination = $.pagination({
...
function groupReport() {
	...
	var jqueryGridPage = 1;
	var jqueryGridRows = 10;
	var url = mobileapirooturl + 'downuseraccountreportlist';
...
function initIndexLottery() {
	...
	var url = mobileapirooturl + 'reportstatistics';
	$.ajax({
		url: url,
		type: "post",
		dataType: "json",
...
function getDownUserNum() {
	var url = mobileapirooturl + 'downuserports';
	$.ajax({
		url: url,
		type: 'POST',
		dataType: 'json',
...
function initEchartsLottery(accountType) {
	...
	var url = mobileapirooturl + 'echarts';
	$.ajax({
		url: url,
		type: 'POST',
		dataType: 'json',
