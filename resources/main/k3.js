...
$(function(){
	...
	$(document).on("click", ".play_select_tit li", function() {
		...
		var htmls='<div class="g_Number_Section_sliderbox sliderbox" style="">'+
            ...
            '</div>'+
          '</div>';
      $('.g_Number_Section').children('.sliderbox').remove();    
      $(".bet-num-box > div."+lottery_code).children('.g_Number_Section').prepend(htmls);
      	
		if(lottery_code == 'k3hzzx'){
			$('.addtobet').hide();
			silbers('k3hz3');
		}else{
			$('.addtobet').show();
    ...
    function loadSilb(){
    	var htmls='<div class="g_Number_Section_sliderbox sliderbox" style="">'+
          ...
          '</div>';
      $('.g_Number_Section').children('.sliderbox').remove();    
      $(".bet-num-box > div.k3hzzx").children('.g_Number_Section').prepend(htmls);
      $('.addtobet').hide();
	  silbers('k3hz3');
    }
    loadSilb();
  ...
  */
  ...
  function silber(name){
    var fands=rates[name].fandian*10;//parseFloat(yrates['tmzx'].fandian)/100;
    var dufrate=parseFloat(rates[name].rate);
    var fandn=Math.round(dufrate)/100;
    lotteryRate=0;
  	lotteryFandian=0;
    $('#minus').on('click',function(){
    ...
    function setslide(data,dufrate,fandn,name){
      ...
      $('.fans').text((fans).toFixed(1));
      $( "#amount" ).text((dufrate-fandl).toFixed(2));
      $(".ball_aid[rate_"+name+"]").text('赔率'+(dufrate-fandl).toFixed(2));
	...
	$('.xiad-left dl').on('keyup','.orderprice',function (){
		setytotal_money();
	})
...
function setytotal_money(){
	...
	$('.orderprice').each(function (i) {
		...
		y_zhushu += isNaN(parseInt($('.ys_zhushu').eq(i).text()))?0:parseInt($('.ys_zhushu').eq(i).text());
		this_y_zhushu = isNaN(parseInt($('.ys_zhushu').eq(i).text()))?0:parseInt($('.ys_zhushu').eq(i).text());
...
var addtotouzhu = function(_playid,number,zhushu,isshow){
	...
	if($Detailnode.length>0){
		...
		var dufrate=parseFloat(rate.rate);
    	var fandn=Math.round(dufrate)/100;
		var rateLotters=(dufrate-(lotteryFandian*fandn).toFixed(2)).toFixed(2);
		var okmoney = accMul(price,rateLotters);
		...
		if(lotteryRate==404){
			var dufrate=parseFloat(rate.rate);
    		var fandn=Math.round(dufrate)/100;
			array['rate']=(dufrate-(lotteryFandian*fandn).toFixed(2)).toFixed(2);
		}else{
			array['rate']=lotteryRate==0?rate.rate:lotteryRate;
		}
		orderList.unshift(array);//push (unshift)ie兼容待测试
	}
	//console.log(orderList);
	var $okamount = accMul(rate.minxf,array.rate);
	...
	html += '<ul>';
	html += '<li>每注<input class="orderprice" value="'+parseInt(rate.minxf)+'" size="5" style="color:black;" onblur="changeorderprice(this.value,\''+trano+'\');" onafterpaste="formatIntVal(this);changebetokmoney(this.value,\''+array.rate+'\',\''+trano+'\');" onkeyup="formatIntVal(this);changebetokmoney(this.value,\''+array.rate+'\',\''+trano+'\');" maxlength="5">元</li>';
...
/*					$.post(apirooturl + 'cpbuy',{"orderList":orderList,'expect':lottery.currFullExpect,'lotteryname':lotteryname}, function(json){
...
var lotterytimes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname/*,'cptype':'ssc'*/}, function(data) {
		if(data.sign==true){
			...
			openLotteryList = $('#openNum_list').find('li');
	...
	$.post(url, {'lotteryname':lotteryname}, function(data) {
		if(data.sign==true){
			...
			for(var o in lotlist){
				...
				html += '<tr><td width="130">'+openinfo.expect+'</td><td width="46" class="c_red fb">'+openinfo.opencode+'</td><td width="40" class="c_blue fb">'+sum+'</td><td>'+smallbig+'&nbsp;|&nbsp;'+oddeven+'</td></tr>';
                ...
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
						...
						if(sum%2!=0)
							...
							oddeven='<em class="bg_blue">双</em>';
						var html = '<tr><td class="lsqh">' + openexpect + '</td><td class="c_red fb ';
						...
						html += '</td><td clsss="c_blue" style="color:#3f81c7;font-weight:bold;">'+sum+'</td><td>'+smallbig+'&nbsp;|&nbsp;'+oddeven+'</td></tr>';
                        if (openCodeListVal.indexOf(openexpect) < 0) {
                                $(".lishi").find('table').find('tbody').prepend(html);
                                $(".lishi").find('table').find('tbody').find('tr').eq(10).remove();
...
var Gameinit = function (_lotteryname) {
	...
	//$('.lot-logo img').attr('src',WebConfigs["ROOT"]+'/resources/images/lot-logo/'+_lotteryname+'.png');
...
function openLottery1(maxnum) {
	...
	// if(openLotteryList.length > 0){
	// 	openLotteryListC = openLotteryList.eq(0).attr('class');
	// 	openLotteryListC = openLotteryListC.substring(0,openLotteryListC.lastIndexOf('_')+1);
	// 	openLotteryList.eq(0).attr('class',openLotteryListC+s_sum1);
	// }else{
	// 	openLotteryList = $('#openNum_list').find('li');
	// }
...
function openLottery2(maxnum) {
	...
	// if(openLotteryList.length > 0){
	// 	openLotteryListC = openLotteryList.eq(1).attr('class');
	// 	openLotteryListC = openLotteryListC.substring(0,openLotteryListC.lastIndexOf('_')+1);
	// 	openLotteryList.eq(1).attr('class',openLotteryListC+s_sum2);
	// }else{
	// 	openLotteryList = $('#openNum_list').find('li');
	// }
...
function openLottery3(maxnum) {
	...
	// if(openLotteryList.length > 0){
	// 	openLotteryListC = openLotteryList.eq(2).attr('class');
	// 	openLotteryListC = openLotteryListC.substring(0,openLotteryListC.lastIndexOf('_')+1);
	// 	openLotteryList.eq(2).attr('class',openLotteryListC+s_sum3);
	// }else{
	// 	openLotteryList = $('#openNum_list').find('li');
	// }
...
//			}
	...
	if (nums.length >= 3) {
		setTimeout(function() {
			var openLotteryList = $('#openNum_list').find('li');
			clearInterval(T3);
...
function setLotterNumber(_index,_nums) {
	...
	openLotteryListC = openLotteryListC.substring(0,openLotteryListC.lastIndexOf('_')+1);
	openLotteryList.eq(_index).attr('class',openLotteryListC+_nums);
}
