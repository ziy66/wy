		...
		<div class="login_input">
			<form method="post" id="form1" class="ruivalidate_form_class" checkby_ruivalidate  onSubmit="return check_form(this)"  action="{:U('Public/register')}">
				...
				<div class="login_pass user_commom_style">
					<span>注册账号：</span>
					<input  type="text"  class="text_accont" placeholder="请输入账号"  style="border-radius:4px;"  id="userName" name="username"  verify="isLoginName" datatype="/[0-9]+/" ajaxurl="{:U('Public/checkusername')}" errormsg="用户名格式错误，可以中文英文字符数字！" nullmsg="请填写用户名！"/>
					<em class="checkInput">
						<i class="iconfont"></i>
						<span></span>
					</em>
				...
				<div class="check_box">
					<input type="checkbox" checked="checked" name="age" />我已经年满18岁，同意勾选。
				</div>
				<div class="btn_submit pull-left" >
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input  type="reset"  style="width:8em;height: 2.5em;" class="btn btn-default reset"  value="重置账户" />　<br/><br/>如果您已经有账号！<a href="{:U('Public/login')}" class="remmber_pwd">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;请点此立即登录吧</a></p>
				</div>
			</form>
		</div>
	</div>
...
<script type="text/javascript">
	$(function(){
		...
		$('.text_accont[name="username"]').blur(function () {
			...
			if(reg.test(text)){
				$(this).siblings('.checkInput').show();
				$(this).siblings('.checkInput').find('span').text('账号不能有汉字');
				$(this).siblings('.checkInput').find('.iconfont').removeClass('icon-chenggong');
				$(this).siblings('.checkInput').find('.iconfont').addClass('icon-cross-ivt');
				return false;
			}
		...
		$('.text_accont[name="password"]').blur(function () {
			...
			if(reg.test(text)){
				$(this).siblings('.checkInput').show();
				$(this).siblings('.checkInput').find('span').text('密码不能有汉字');
				$(this).siblings('.checkInput').find('.iconfont').removeClass('icon-chenggong');
				$(this).siblings('.checkInput').find('.iconfont').addClass('icon-cross-ivt');
				return false;
			}
		...
		$('.text_accont[name="tradepassword"]').blur(function () {
			...
			if(reg.test(text)){
				$(this).siblings('.checkInput').show();
				$(this).siblings('.checkInput').find('span').text('资金密码不能有汉字');
				$(this).siblings('.checkInput').find('.iconfont').removeClass('icon-chenggong');
				$(this).siblings('.checkInput').find('.iconfont').addClass('icon-cross-ivt');
				return false;
			}
...
<style type="text/css">
body {background-image: url(assets/bg2.jpg);}
</style>
</body>
</html>