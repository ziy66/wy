...
$(function () {
  ...
  if ($(".choice_lottery_playdetail_left").size() > 0) {
    $(".choice_lottery_playdetail_left").click(function () {
      $(".play_select_container").toggle();

      if ($('.play_select_container').hasClass('yyplay_select_container')) {
        $(this).find('.icon-sanjiaoxing').css('transform', 'rotate(360deg)');
        $('.play_select_container').removeClass('yyplay_select_container')
      } else {
        $(this).find('.icon-sanjiaoxing').css('transform', 'rotate(180deg)');
        $('.play_select_container').addClass('yyplay_select_container')
      }
    })
    ...
    $(".play_select_tit li").click(function () {
      if ($("#j_play_select").hasClass("show_child_PlayCont")) {
        ...
        var _theTitle = $(this).find('.lineMore-item').text();
        $(".choice_playName").eq(0).empty().text(_theTitle);
        $(".play_select_container").hide();
        $('.choice_lottery_playdetail_left').find('.icon-sanjiaoxing').css('transform', 'rotate(180deg)');
        $('.play_select_container').addClass('yyplay_select_container');
      }
  ...
  $(document).on('click', '#k3hzzx .ball_number,#k3sthtx .ball_number,#k3slhtx .ball_number', function () {
    ...
    if (playid == 'k3slhtx') {
      ...
      $("#k3hzzx .ball_number.curr").each(function (index) {
        ...
        var _number = $(this).attr('ball-number');
...
function lottery_yuji_money(rate) {
  ...
  okmoney = okmoney.toFixed(2);
  console.log(_ratearray);
  ...
  $.post(url, {'lotteryname': lotteryname}, function (data) {
    if (data.sign == true) {
      ...
      for (var o in lotlist) {
        ...
        var opencode_dice='<t class="dice dice'+array[0]+'"></t><t class="dice dice'+array[1]+'"></t><t class="dice dice'+array[2]+'"></t>';
        ...
        html += '<tr style="color:#caebda;"><td class="fz13">' + openinfo.expect + '</td><td class="fb">' + opencode_dice + '</td><td class="fb">' + sum + '</td><td>' + smallbig + '&nbsp;|&nbsp;' + oddeven + '</td></tr>';
    ...
    CDTime = setInterval(function () {
      ...
      if (t >= 0) {
        ...
        way.set("gametimes.s", s1 + s2);
		  ...
		  nextNotice($('#f_lottery_info_number').html());
        //audioPlay(2);
        clearInterval(CDTime);
...
function setDice(name, value) {
  var $curDice = $('.openNum_list').find('t[way-data="' + name + '"]');
  var i = 0;
  try {
    i = parseInt(value);
  } catch (err) {
  }
  var new_class = "dice" + i;
  $curDice.removeClass().addClass("dice").addClass(new_class);
}
function setRDice(name, value) {
  var $curDice = $('.openNum_list').find('t[way-data="' + name + '"]');
  var i = 0;
  try {
    i = parseInt(value);
...
function openLottery(ball, maxnum) {
  if (T10) {
    clearInterval(T10);
    setDice("showExpect.openCode10", " ");
  }
  if (T9) {
    clearInterval(T9);
    setDice("showExpect.openCode9", " ");
  }
  if (T8) {
    clearInterval(T8);
    setDice("showExpect.openCode8", " ");
  }
  if (T7) {
    clearInterval(T7);
    setDice("showExpect.openCode7", " ");
  }
  if (T6) {
    clearInterval(T6);
    setDice("showExpect.openCode6", " ");
  }
  if (T5) {
    clearInterval(T5);
    setDice("showExpect.openCode5", " ");
  }
  if (T4) {
    clearInterval(T4);
    setDice("showExpect.openCode4", " ");
  }
  if (T3) {
    clearInterval(T3);
    setDice("showExpect.openCode3", " ");
  }
  if (T2) {
    clearInterval(T2);
    setDice("showExpect.openCode2", " ");
  }
  if (T1) {
    clearInterval(T1);
    setDice("showExpect.openCode1", " ");
  }
...
function openLottery1(maxnum) {
  setRDice("showExpect.openCode1", maxnum);
}

function openLottery2(maxnum) {
  setRDice("showExpect.openCode2", maxnum);
}

function openLottery3(maxnum) {
  setRDice("showExpect.openCode3", maxnum);
}

function openLottery4(maxnum) {
  setRDice("showExpect.openCode4", Math
      .round(Math.random() * (maxnum - 1) + 1));
}

function openLottery5(maxnum) {
  setDice("showExpect.openCode5", Math
      .round(Math.random() * (maxnum - 1) + 1));
}
function openLottery6(maxnum) {
  setRDice("showExpect.openCode6", Math
      .round(Math.random() * (maxnum - 1) + 1));
}

function openLottery7(maxnum) {
  setRDice("showExpect.openCode7", Math
      .round(Math.random() * (maxnum - 1) + 1));
}

function openLottery8(maxnum) {
  setRDice("showExpect.openCode8", Math
      .round(Math.random() * (maxnum - 1) + 1));
}
function openLottery9(maxnum) {
  setRDice("showExpect.openCode9", Math
      .round(Math.random() * (maxnum - 1) + 1));
}
function openLottery10(maxnum) {
  setRDice("showExpect.openCode10", Math.round(Math.random() * (maxnum - 1) + 1));
}
// 停止开奖
function stopLottery(codes) {
  ...
  if (nums.length >= 10) {
    setTimeout(function () {
      clearInterval(T10);
      setDice("showExpect.openCode10", nums[9] + "");
//			if(nums.length==10){
//				showLottery();
//			}
    }, 4000);
  ...
  if (nums.length >= 9) {
    setTimeout(function () {
      clearInterval(T9);
      setDice("showExpect.openCode9", nums[8] + "");
//			if(nums.length==9){
//				showLottery();
//			}
    }, 4000);
  ...
  if (nums.length >= 8) {
    setTimeout(function () {
      clearInterval(T8);
      setDice("showExpect.openCode8", nums[7] + "");
//			if(nums.length==8){
//				showLottery();
//			}
    }, 4000);
  ...
  if (nums.length >= 7) {
    setTimeout(function () {
      clearInterval(T7);
      setDice("showExpect.openCode7", nums[6] + "");
//			if(nums.length==7){
//				showLottery();
//			}
    }, 3500);
  ...
  if (nums.length >= 6) {
    setTimeout(function () {
      clearInterval(T6);
      setDice("showExpect.openCode6", nums[5] + "");
//			if(nums.length==6){
//				showLottery();
//			}
    }, 3000);
  ...
  if (nums.length >= 5) {
    setTimeout(function () {
      clearInterval(T5);
      setDice("showExpect.openCode5", nums[4] + "");
      // if(nums.length==5){
      // 	showLottery();
      // }
    }, 2500);
  ...
  if (nums.length >= 4) {
    setTimeout(function () {
      clearInterval(T4);
      setDice("showExpect.openCode4", nums[3] + "");
//			if(nums.length==4){
//				showLottery();
//			}
    }, 2000);
  ...
  if (nums.length >= 3) {
    setTimeout(function () {
      clearInterval(T3);
      setDice("showExpect.openCode3", nums[2] + "");
//			if(nums.length==3){
//				showLottery();
//			}
    }, 1000);
  ...
  if (nums.length >= 2) {
    setTimeout(function () {
      clearInterval(T2);
      setDice("showExpect.openCode2", nums[1] + "");
//			if(nums.length==2){
//				showLottery();
//			}
    }, 1000);
  ...
  if (nums.length >= 1) {
    setTimeout(function () {
      clearInterval(T1);
      setDice("showExpect.openCode1", nums[0] + "");
//			if(nums.length==1){
//				showLottery();
//			}
    }, 1000);
...
// Remilia 是最萌哒！ //remilia.cc
// Remilia 是最萌哒！ //remilia.cc
nextNotice = function(now){
	html = '<div class="msgbox" style="background:  #fff;border-radius:  5px;width: auto;height: auto;position:  fixed;top: 100px;z-index:  1000000;left: 40px;right: 40px;text-align: center;"><p style="padding-bottom: 10px;border-bottom: 1px solid #d8d8d8;">温馨提示</p><p style="padding: 10px;border-bottom: 1px solid #d8d8d8;">'+now+'期已截止<br>当前期号<font color="red">'+(now+1)+'</font><br>投注时请注意期号</p><p style="color: #609eff;" onclick="$(\'.msgbox\').hide();">了解</p></div>';
	$('body').append(html);
}
//this kehu is a sb
$(function(){
	$('.iconfont', $('.button.button-link.button-nav.pull-left.bar-nav-top-left').attr('href','javascript:history.go(-1)')).removeClass('icon-shouyeshouye1').html('﹤').css('font-size','30px').css('font-weight','bolder');
	$('#Game_CheckBall').css('padding','25px');
	$('body').append('<style>.ball_list_ul .ball_item {padding: 0 5px 10px 5px;}.ball_list_ul .ball_item a {padding: initial;}</style>');
});
